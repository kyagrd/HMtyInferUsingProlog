...
This programming technique is known as \emph{delayed goals} in logic programming,
which is like building up a to-do list or a continuation. We use
the Definite Clause Grammar (DCG) rules \cite{SWIPrologManual} to collect delayed
goals using a neat syntax. The DCG rules were originally designed for describing
production rules of formal grammar, where nonterminals are specified within
the brackets and context-sensitive conditions are specified within curly braces
using ordinary Prolog predicates. Here, we exploit DCG rules (as many other do)
as a neat syntax for a \emph{writer monad} that collects \texttt{kind} assertions
