...
theory of type polymorphism in late '70s. We progressively extend
...
before running the Prolog specifications. {\small\vspace*{-1ex}
\begin{verbatim}
  :- set_prolog_flag(occurs_check,true).
  :- op(500,yfx,$).
\end{verbatim} {\small\vspace*{-.7ex} }\noindent
...
For instance, \verb|E1$E2| is an application of \verb|E1| to \verb|E2|.
...
represented as \verb|mono(A)| and a polymorphic type is represented as
\verb|poly(C,A)|. A typing context in HM may contain two kinds of bindings:
monomorphic bindings (\verb|X:mono(A)|) and
...
a machine executable reference implementation. We can run the specification
for type checking, as follows: {\small \vspace*{-1ex}
\begin{verbatim}
...
\begin{itemize}
\item[1.]
The 1st line of the definition is the first stage at the typing level.
For example, {\small \vspace*{-1ex}
\begin{verbatim}
...
\item[2.] From the 2nd line,
we invoke this delayed goal after preprocessing of concertizing
the type variables. In the 2nd line, we make a copied version of
...
and so on. After 5th line, where it is concretized as
...
registered to the kinding context because the first kinding rule requires
that type variables must be bound in the typing context. The 6th line creates
monomorphic type bindings for all the variable names in {\small\verb|Xs|},
and collects them into {\small\verb|KC1|}. Continuing with the identity example,
{\small\verb|KC1=[t1:mono(K1)]|} after the 6th line. The 7th line
extends each kinding context in the dealyed goal {\small\verb|Gs|} with
the bindings ({\small\verb|KC1|}) for the freshly generated variables, 
where the goals with extended contexts are collected in {\small\verb|Gs1|}.
After 7th line, we have 
{\small\verb|Gs1 = [kind([t1:mono(K1)], (var(t1)->var(t1)), o)]|}.
\item[8.] Finally, the delayed well-kindedness assertions in \verb|Gs1|
  are called on as goals, which amounts to the following query
  for our identity function example:{\small
\begin{verbatim}
  ?- kind([t1:mono(K1)], (var(t1)->var(t1)), o).
  K1 = o
\end{verbatim} \vspace*{-.7ex} }
It succeeds by inferring that the fresh
type variable \verb|var(t1)| must have kind \verb|o|. \vspace*{-1.5ex}
\end{itemize}
...
\end{itemize}
Extending our relational specification with term indices would be
...
the possibility of relational specification for advanced polymorphic features,
...
specifications of advanced type systems. Such a tool can open a new
era in language construction, analogous to the impact when parser generators,
...
seamlessly put together such novel features/designs accomplished individually
