...
of polymorphism, and that logic programming could be even more effective
for specifying type inference when equipped with better theories and
...
\]
If \textit{id} had been bound monomorphically, it would be ill-typed
because
...
is emphasized as \verb|mono(A)|, which corresponds to simply \verb|A|
...
the original term except all the Prolog variables has been substituted with
...
We gave explicit type signatures above for the sake of smooth explanation,
but the type of \texttt{mkToSet} can be completely inferred from
its definition, without the type signature of \texttt{mkToSet}.
...
Because the kinding rules require kinding contexts. The typing rules
...
\begin{verbatim}
  ...
  infer_type(KC,C,E,T) :-
    ...
    findall(A:mono(K), member(var(A),Xs), KC1), % kind binidngs for var(t)
...
places among the typing rules. A generic theory of typed lambda calculus
[TODO cite Pure Type System] indicates that well-kindednss checks
are required at the formation of function types, that is,
in the typing rule for lambda expressions (the second rule of \verb|type|).
...
are threaded through the recursive definition of the \verb|type| predicate.
...
In Fig.\ref{fig:patlam}, we specify pattern matching expressions without
the scrutinee, which is also known as pattern-matching lambdas.
A pattern lambda is a function that awaits an expression to be passed in
...
with type constructor, kind polymorphism, and non-nested pattern matching
...
a \emph{non-regular datatype}, also known as a \emph{nested datatype}
because the types of recursive components typically become nested as
...
user supplied type annotations. In Fig.\ref{fig:mit}, we specify
a subset of a functional language that supports a recursion scheme, which
naturally generalize from regular datatypes to non-regular datatypes.
In particular, we specify the Mendler-style iteration [cite mit paper]
supported in the Nax language [cite Nax]. In Nax, all recursive
constructs, both at the type level and at the term level, are
defined using the primitives provided by the language, avoiding
uncontrolled general recursion. The \verb|mu(F)| appearing in
the Prolog specification corresponds to the recursive types $\mu F$
...
[cites on Mendler-style recursion] and Nax [cite Nax] are available
in the references section at the end of this paper.

The specification for Mendler-style iteration is only 20 lines,
including pattern-matching lambdas $16+20=36$ lines, including
HM with type constructor polymorphism and kind polymorphism
...
handling unification variables and symbols
(e.g., \verb|copy_term| or \verb|gensym|) could help us express
the concept of local constraints as elegantly as expressing
polymorphic instantiation. In addition, more user supplied
annotations will be required to guide the type inference,
even for pattern-matching lambdas that do not involve recursion,
because GADT type inference is undecidable by itself regardless
of polymorphic recursion. In Nax, pattern-matching lambdas can
have annotations, which are just like the annotations
on the Mender-style iteration in \S\ref{ssec:mit}.
...
\subsubsection*{Acknowledgments.} Special thanks to Andrea Vezzosi
for posting the HM implementation in Prolog and also to the Haskell
IRC channel users who would have been involved in coming up with the idea;
that 7 lines of code provided additional motivation for this case study.
...
\bibitem{urlHM} Andrea Vezzosi, Hindley Milner in 7 lines of hacky prolog,
\url{http://lpaste.net/65035}
