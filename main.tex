...
\title{A Case Study on Polymorphic Type Inference using Prolog (draft submitted to APLAS '15)}
...
Recently (2nd week of Aug) found out that there needs additional change when
we have \verb|inst_type| for type instantiation with kind sanity checking.
There are two places to change: type rule for applications
(\verb|type(KC,C,X$Y,...) :- ...|) and type rule for
case alternatives (\verb|type_alt|). While fixing this bug
typing rule mit for non-regular datatype has been a bit simplified
(eliminated the \verb|Es| variable).
